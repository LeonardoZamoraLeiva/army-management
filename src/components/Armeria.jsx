...
export default function Armeria() {
    ...
    // --- NUEVOS ESTADOS DE FILTRO TÁCTICO ---
    ...
    useEffect(() => {
        const revisarAtajos = () => {
            ...
            if (targetEscuadron) {
                ...
                setSoldadoId(''); // Limpiamos para que vea la lista filtrada
    ...
    const soldadosFiltrados = soldados.filter(s => {
        ...
        // --- LÓGICA DE FILTRADO BUG-PROOF ---
        // Buscamos si este soldado aparece en algún escuadrón (como líder o miembro)
        ...
        const matchEscuadron = filtroEscuadron === '' || (
            ...
            ? !escAlQuePertenece // Es reserva si no está en ningún escuadrón
            : escAlQuePertenece?.id === filtroEscuadron // Coincide con el ID seleccionado
        );
    ...
    const statsHome = {
        ...
        rareza: { comun: equipo.filter(e => (e.rareza || 'Común') === 'Común').length, poco_comun: equipo.filter(e => e.rareza === 'Poco Común').length, raro: equipo.filter(e => e.rareza === 'Raro').length, muy_raro: equipo.filter(e => e.rareza === 'Muy Raro').length, legendario: equipo.filter(e => e.rareza === 'Legendario').length }
    };
    ...
    return (
        <div style={{ display: 'flex', gap: '20px', animation: 'fadeIn 0.3s ease' }}>
            ...
            <div className="estacion-equipamiento" style={{ flex: 1.5 }}>
                ...
                {/* --- NUEVO PANEL DE FILTROS TÁCTICOS --- */}
